...
import com.nesterov.university.service.exceptions.*;
...
public class StudentService {
    ...
    public void create(Student student) {
        ...
        if (studentDao.findByGroupId(student.getGroupId()).size() <= maxGroupSize) {
...
//		verifyAddressIsUnique(student);
    ...
    private void verifyMaxGroupSizeIsExceeded(Student student) {
        if (studentDao.findByGroupId(student.getGroupId()).size() <= maxGroupSize) {
            String message = format("Exceeded the maximum group size with id = '%s'", student.getGroupId());
            throw new ExceededMaxGroupSizeException(message);
        }
    }
}
